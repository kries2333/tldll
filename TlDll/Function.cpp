...
TAsmMonster CFunction::FUN_GetMonsterByName(_tstring MonsterName, int nMonsterType, int nKillType)//名称获取距离自己最近的怪物
{
	...
	for (auto m : AMonster)
	{
		if (((CString)m.szName).Find(MonsterName.c_str(), 0) != -1)
		{
			//if (((CString)m.szBaseName).Find("头目", 0) == -1 &&
			//	((CString)m.szBaseName).Find("头领", 0) == -1 &&
			//	((CString)m.szBaseName).Find("首领", 0) == -1)
			{
				if (m.nAttack == 0)
				{
					switch (nMonsterType)
...
bool CFunction::FUN_MovToMonster(TAsmMonster tAsmMonster, TAsmSkill tAsmSkill)//走到技能射程范围内
{
	...
	//FUN_RunToTarget(tAsmMonster.fMonsterX, tAsmMonster.fMonsterY, fr);

	FUN_RunTo(tAsmMonster.fMonsterX, tAsmMonster.fMonsterY, 30);

	for (int i = 0; i < 10 && g_pMain->bRun; i++)
	{
...
// 攻击多个怪物
void CFunction::FUN_AttackMonsterEx(VUserMonsterName monsterNames)
{
	if (!g_pMain->bRun)
	{
		return;
...
{
	g_pAsmSkill->AsmUseSkillCall(-1, 21);//使用此技能
}
...
void CFunction::FUN_AutoBuy(_tstring itemNames, _tstring sceneName, int nPosX, int nPosY, _tstring npcName, _tstring _talkName)
{
	auto  vString = UserSubMonsterName(itemNames, _T('|'));
...
//卖物品
void CFunction::FUN_AutoSell(_tstring itemNames)
{
	//获取当前场景
	float fPosX = 0.0;
	float fPoxY = 0.0;
	_tstring npcName = "";
	TAsmMap map = role_curmap();
	switch (map.nSceneId)
	{
	case 0:	//洛阳
	{
	...
	if (FUN_RunToTargetEx(fPosX, fPoxY, map.nSceneId)) {

		g_pMsg->SetAutoRunTargetNPCName(npcName.c_str()); //移动到NPC
		if (g_pMsg->IsNpcDialog())
		{
			g_pMsg->msg_dostring("ClickMission(\"购买药品\")");//msg的lua消息函数
			Sleep(2000);

			g_pAsmItem->AutoSell(itemNames);
		}
	}
}
