...
BOOL CFunction::FUN_RunToTargetEx(float fx, float fy, int SceneId, float dis)//跨地图寻路
{
	...
	while (g_pMe->bRun)
	{
		g_pMe->bPauseProtect = true;	//暂停吃药保护
		...
		if (tRoleInfo.nState != 2)
		{
			if (!bMov)
			{
				...
				{
					dbgPrint("开始本地寻路 %f, %f, %d", fx, fy, SceneId);
		...
		if ((dwCurTime - dwEndTime) > (5 * 1000)) //如果5秒没有移动就重新移动
		{
			auto APos = g_pAsmRole->GetPos();
			if (((int)APos.fx == nOldX) &&
				((int)APos.fy == nOldY))
			{
				dbgPrint("5秒没有移动就重新移动");
				bMov = false;
			}
		...
		{
			dbgPrint("到达目的地");
			g_pMe->bPauseProtect = false; //启动吃药保护
			return TRUE;
		}
...
void CFunction::FUN_JoinRace(CString strRaceName, CString strNpcName, int sceneId, float x, float y)
{
	...
	dbgPrint("FUN_JoinRace 当前门派=%d 待加入门派=%s npc=%s 场景=%d 坐标x=%d y=%d", menpai, strRaceName, strNpcName, sceneId, x, y);
	if (menpai == 9) //判断是否是无门派
	{
		...
		while (g_pMe->bRun) {
			...
			if (FUN_RunToTargetEx(x, y, sceneId))
			{
				g_pMsg->SetAutoRunTargetNPCName(strNpcName); //移动到NPC

				g_pMsg->msg_dostring("ClickMission('加入门派')");//msg的lua消息函数
				Sleep(500);

				_tstring szTemp = "我确定要拜入" + strRaceName + "派";
...
void CFunction::FUN_AutoBuy(CString itemNames, CString sceneName, int nPosX, int nPosY, CString npcName, CString _talkName)
{
	...
	for (auto temp : szTemps)
	{
		...
		if (items.size() == 2)
		{
			int BagItemNum = g_pAsmItem->AsmGetItemNum(items[0].c_str());
			int BugItemNum = stoi(items[1].c_str());
			dbgPrint("%s当前背包=%d 需要购买数量=%d", items[0].c_str(), BagItemNum, BugItemNum);
			if (BugItemNum > BagItemNum)
			{
				if (sTempNames != "")
				{
					CString ss;
					ss.Format("%s=%s", items[0].c_str(), items[1].c_str());
					sTempNames = sTempNames + "|" + ss;
				}
				else
				{
					sTempNames.Format("%s=%s", items[0].c_str(), items[1].c_str());
				}
	...
	dbgPrint("需要购买得物品数据=%s", sTempNames);
...
void CFunction::FUN_UseAttackSkill(TUserSkill tUserSkill/*CString SkillName*/, TAsmMonster tAsmMonster)//使用攻击技能
{
	...
	//		{
	//			dbgPrint("使用技能%s 技能Id=%d", tUserSkill.tAsmSkill.szName, tUserSkill.tAsmSkill.nSkillId);
	//			FUN_UseMovAttackSkill(tUserSkill.tAsmSkill, tAsmMonster);//使用攻击技能
	//		}
	//	}
	//}
